...
class InsertIntoHiveTableHarvester extends Harvester[InsertIntoHiveTable] with Logging{
  override def harvest(node: InsertIntoHiveTable): AtlasEntity.AtlasEntitiesWithExtInfo = {
    //解析出目标表
    val tableName = node.table.identifier.table
    var database= node.table.identifier.database.getOrElse("default")
