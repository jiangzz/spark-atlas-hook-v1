...
import org.apache.spark.sql.catalyst.expressions.{Concat, Expression, NamedExpression}
...
class CreateHiveTableAsSelectCommandHarvester extends Harvester[CreateHiveTableAsSelectCommand] with Logging {
  override def harvest(node: CreateHiveTableAsSelectCommand): AtlasEntity.AtlasEntitiesWithExtInfo = {
    //解析出目标表
    ...
    logInfo(s"创建表血缘:${resultTable} ->  ${inputTables.mkString(" , ")}")
    //完成Hive表的血缘解析
  ...
  def harvestColumnLineage(database:String,table:String,cluster:String,shortColumn:String,resultColumn:String,inputColumns:Seq[String], expression:String,hiveProcess:AtlasEntity):AtlasEntity={
  ...
  def harvestTableLineage(resultTable:String,inputTables:Seq[String]):AtlasEntity={
    ...
    if(querySQL==null || querySQL==""){
      querySQL="CREATE_TABLE_AS_SELECT"
    }
    atlasEntity.setAttribute("operationType", "CREATETABLE_AS_SELECT")
